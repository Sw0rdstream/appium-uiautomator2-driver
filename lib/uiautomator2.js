...
class UiAutomator2Server {
  ...
  async installServerApk (installRetries) {
    ...
    try {
      ...
      logger.error(`Unable to find instrumentation target '${INSTRUMENTATION_TARGET}'`);
      if (output) {
  ...
  async startSession (caps) {
    ...
    //   }
    // }
    let retries = getRetries(caps.uiautomator2ServerLaunchTimeout, 'Server launch', SERVER_LAUNCH_RETRIES);
