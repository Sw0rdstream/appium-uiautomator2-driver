...
class UiAutomator2Server {
  ...
  async installServerApk () {
    ...
    // if (!await this.checkInstrumentation()) {
    //   logger.debug('Unable to find instrumentation process. Trying to uninstall server');
    //   isApkInstalled = isTestApkInstalled = false;
    //   await this.uninstallServerAndTest(apkPackage, testApkPackage);
    // }
    ...
    logger.debug(`Waiting for instrumentation to be available`);
    let i = 0;
    await retryInterval(10, 1000, async () => {
      logger.error(`TRY #: ${i++}`);
      await this.checkInstrumentation();
    });
  }
  ...
  async checkInstrumentation () {
    logger.debug('Checking instrumentation status');
    let output = await this.adb.shell(['pm', 'list', 'instrumentation']);
    logger.debug(`Available instrumentation processes:`);
    for (let line of output.split('\n')) {
      logger.debug(`    ${line.replace('instrumentation:', '')}`);
    }
    if (output.indexOf(INSTRUMENTATION_TARGET) === -1) {
      logger.error(`Instrumentation target '${INSTRUMENTATION_TARGET}' not found`);
      return false;
    }
    return true;
  }
